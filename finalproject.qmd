...
```{r}
transit_original <- read_csv("finalproject.csv")
...
\\01. Cluster one covers downtown, Capitol Hill, and Georgetown.

\\02.Cluster two is the most geographically dispersed, and it captures all of Wards 7 and 8.

\\03.Cluster three is intermingled with much of cluster two, but covers more of the northern areas of DC.

\\04.Cluster four largely represents Haines Point and the National Mall.

```{r}

# Plot clusters by geography
...
**Cluster two is "Fewer options, restricted traffic."** This cluster may represent census tracts that have fewer transportation modes but restrictions on truck traffic, such as low-transit residential areas.
...
To further test these interpretations, we use data visualization.
...
Traffic volume - moved here from the data cleaning file - don't have to keep!
...
Public Transit - moved here from the data cleaning file - don't have to keep!
...
In the third regression, we use all of the numeric variables we have as covariates, aiming to identify some of the specific tranportation characteristics that may be making the first cluster significant. The factors that are most significantly associated with reductions in PM 2.5 are bus stops with bike racks, commuting by walking, and lower traffic volume.
...
As expected from our regression analysis, air pollution does vary spatially. Each of the air monitors detects different levels of each pollutant--not just PM 2.5. And example of variation in NO2 pollution across the air monitors is below.
...
Moreover, air pollution varies over time--something we were not able to test with the 2017 data. For example, ozone levels drop throuhgout the first third of the year before increasing again, and are typically highest in December.
...
Finally, we create a decision tree that categorizes air monitor observations. Notably, the first two splits are based on time--another reason to advocate for this type of data in every census tract.
